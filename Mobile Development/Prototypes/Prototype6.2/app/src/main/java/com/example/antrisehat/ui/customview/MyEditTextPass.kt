...
class MyEditTextPass : TextInputEditText, View.OnTouchListener {
    ...
    private lateinit var eveIcon: Drawable
    ...
    private fun init(){
        eveIcon = ContextCompat.getDrawable(context, R.drawable.ic_eye_off) as Drawable
    ...
    private fun showEyeButton(){
        setButtonDrawables(endOfTheText = eveIcon)
    }
    ...
    override fun onTouch(v: View?, event: MotionEvent): Boolean {
        if (compoundDrawables[2] != null) {
            ...
            if (layoutDirection == View.LAYOUT_DIRECTION_RTL) {
                eyeButtonEnd = (eveIcon.intrinsicWidth + paddingStart).toFloat()
                if (event.x < eyeButtonEnd) isEyeButtonClicked = true
            } else {
                eyeButtonStart = (width - paddingEnd - eveIcon.intrinsicWidth).toFloat()
                ...
                return when (event.action) {
                    MotionEvent.ACTION_DOWN -> {
                        ...
                        if (transformationMethod.equals(HideReturnsTransformationMethod.getInstance())) {
                            ...
                            eveIcon = ContextCompat.getDrawable(context, R.drawable.ic_eye) as Drawable
                            showEyeButton()
                        }
                        true
                    }
